...
async def gerar_senha(request: GeraSenhaRequest):
    ...
    try:
        ...
        generator.salvar_senha(senha, 'random', request.tamanho)
        return {"senha": senha, "tipo": "random"}
...
async def gerar_passphrase():
    ...
    generator.salvar_senha(passphrase, 'passphrase')
    return {"senha": passphrase, "tipo": "passphrase"}
...
async def listar_historico():
    ...
    user_id = "guest-user"
    generator = PasswordGenerator(user_id)
    return {"senhas": generator.listar_senhas()}